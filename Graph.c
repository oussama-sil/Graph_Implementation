...
void libererArc(Noeud u, Noeud v){
    ...
    if(stop==1){ //arc trouve dans p et son prec est q
        ...
        dec_deg_noeud(u);
        dec_deg_out_noeud(u);
        dec_deg_noeud(noeud_arc(p));
        dec_deg_in_noeud(noeud_arc(p));
        liberer_arc(p);
    }
}
...
void afficher_arc_noeud(Noeud u){
    printf("Arc noeuds : { nb_arcs : %d , (info_arc,noeuds) : ",deg_out(u));
...
void afficher_adjacents_noeud(Noeud u){
    printf("Adjacents : { nb_arcs : %d , noeuds : ",deg_out(u));
    Noeud p = liste_arc_noeud(u);
    while(p != NULL){
